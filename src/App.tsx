// src/App.tsx  (Option B: flat serverless route at /api/insert)
...
const parseDateMDY = (raw: string) => {
  const t = raw.trim();
  return /^\d{2}\/\d{2}\/\d{4}$/.test(t) ? t : undefined;
};
...
const App: React.FC = () => {
  ...
  // server proxy call to /api/insert (flat)
  ...
  const handleSubmitToCaspio = async () => {
    ...
    try {
      ...
      const submissionData: ExtractedData = {
        ...
        Quote_pdf: `/${fileName}` // reference string only
      };
  ...
  const DataDisplay: React.FC<{ data: ExtractedData }> = ({ data }) => {
    const formatValue = (value: any): string => (value == null ? '' : String(value));
    return (
