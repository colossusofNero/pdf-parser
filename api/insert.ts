...
async function getSheetsClient() {
  const google = await dynamicGoogle();
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, updatedRange: append.data.updates?.updatedRange, updatedRows: append.data.updates?.updatedRows });
  } catch (e: any) {
